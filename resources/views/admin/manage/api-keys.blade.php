...
<div class="card">
    <div class="table-wrap">
        <table>
            ...
            <tbody id="keys-tbody">
                <tr><td colspan="10"><div class="loading-overlay"><div class="spinner"></div> Loading...</div></td></tr>
            </tbody>
        </table>
    </div>
...
function integrationBadge(value) {
    if (!value) return '<span style="color:var(--text-muted)">-</span>';

    const styles = {
        enabled: ['#d1fae5', '#065f46', 'Enabled'],
        disabled: ['#fee2e2', '#991b1b', 'Disabled'],
        approved: ['#d1fae5', '#065f46', 'Approved'],
        pending: ['#fef3c7', '#92400e', 'Pending'],
        rejected: ['#fee2e2', '#991b1b', 'Rejected'],
    };

    const [bg, color, label] = styles[value] || ['#e2e8f0', '#475569', value];
    return `<span style="display:inline-flex;align-items:center;padding:3px 9px;border-radius:999px;background:${bg};color:${color};font-size:11px;font-weight:700">${label}</span>`;
}
...
async function loadKeys(page = 1) {
    ...
    if (!keys.length) {
        tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;color:var(--text-muted);padding:24px">No API keys found</td></tr>';
    ...
    tbody.innerHTML = keys.map(k => `<tr>
        ...
        <td>${integrationBadge(k.user?.latest_integration?.status)}</td>
        <td>${integrationBadge(k.user?.latest_integration?.admin_status)}</td>
        ...
        <td>
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                ${k.is_active ? '<span class="txn-status success">active</span>' : '<span class="txn-status failure">revoked</span>'}
